...
def write_mapped_abaqus_input(
    mesh_path: str | Path,
    material_path: str | Path,
    output_path: str | Path,
) -> None:
    """Write a self-contained simple Abaqus input with mesh and BV/TV materials.

    The generated file is intended for simple Abaqus input files where section
    definitions can be appended after the node and element blocks. For complex
    files using *Part/*Assembly scoping, users may need to include the generated
    material section file at the correct location in their Abaqus model.
    """
    mesh_text = Path(mesh_path).read_text(encoding="utf-8")
    material_text = Path(material_path).read_text(encoding="utf-8")
    with Path(output_path).open("w", encoding="utf-8") as handle:
        handle.write("** Self-contained BV/TV-mapped Abaqus input generated by map_bvtv_to_fe.py\n")
        handle.write("** Source mesh: " + str(mesh_path) + "\n")
        handle.write("** Source material mapping: " + str(material_path) + "\n")
        handle.write("**\n")
        handle.write(mesh_text.rstrip())
        handle.write("\n\n")
        handle.write("** BV/TV-derived material and section assignments\n")
        handle.write(material_text.rstrip())
        handle.write("\n")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--no-mapped-inp",
        action="store_true",
        help="Do not write the self-contained <out-prefix>.mapped.inp Abaqus file.",
    )
    parser.add_argument(
        "--landmarks-json",
        help=(
...
def main(argv: list[str] | None = None) -> int:
    ...
    mapped_inp_path = out_prefix.with_suffix(".mapped.inp")
    ...
    if not args.no_mapped_inp:
        write_mapped_abaqus_input(args.mesh, inp_path, mapped_inp_path)
